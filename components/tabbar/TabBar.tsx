...
const TabBar = (tabBarProps : TabBarProps) => {
  ...
  const customizeIcon = (focused : boolean , defaultIconName : any , focusedIconName : any ) => {
      return <Ionicons 
          ...
          size={24}
          color={focused ? COLORS.primary_color : theme.tab_color }
      />
  }

  const customizeLabel = (focused : boolean , labelText : string) => {
      return <Text style={{color : focused ?  COLORS.primary_color : theme.tab_color}} > {labelText} </Text>
  }
    ...
    <View style={styles.container} >
        ...
        <Tab.Navigator
          ...
          screenOptions={{
            ...
            headerShown : tabBarProps.headerShown as boolean,
            headerTitle : ()=> {
